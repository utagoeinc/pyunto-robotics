...
PLACES: dict[str, tuple[float, float, float, float]] = {
    # -0.05 clears the mattress everywhere.
    #
    # -0.09 put her *lowest measured centre* at the surface, which is not the same thing:
    # a capsule's underside is a radius below its centre, so the hips and the heels still
    # cut 2-3 cm into the bedding while the rest lay on it. Measured per part, not averaged.
    #
